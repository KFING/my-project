...
private:
    ...
    void ProcessOperation(std::stack<double>& st, char oper)
    {
        if (oper < 0) {
            ...
            switch (-oper)
            {
            case '+': st.push(arg); break;
            case '-': st.push(-arg); break;
            case '!': st.push(fact(arg)); break;

            default: assert(!"invalid unary operation"); break;
            }
        }
        else {
            double arg1 = st.top(); st.pop();
            double arg2 = st.top(); st.pop();
            ...
            case '*': st.push(arg2 * arg1); break;
            case '/': st.push(arg2 / arg1); break;
            case '%': st.push(std::fmod(arg2, arg1)); break;
            case '^': st.push(std::pow(arg2, arg1)); break;

            default: assert(!"invalid binary operation"); break;
            }
        }
    }
